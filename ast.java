...
class ProgramNode extends ASTnode {
    ...
    public void nameAnalysis() {
        ...
        myDeclList.nameAnalysis(symTab, 1); //1 is calling from global space
    }
...
class DeclListNode extends ASTnode {
     ...
     */
    public void nameAnalysis(SymTable symTab, int lg) {
        nameAnalysis(symTab, symTab, lg);
    }
     ...
     * decls in the list.
     */
    public void nameAnalysis(SymTable symTab, SymTable globalTab, int lg) {
        for (DeclNode node : myDecls) {
			if (lg == 1) {
				symTab.offset = -8;
			}
            if (node instanceof VarDeclNode) {
				...
				IdNode var = ((VarDeclNode)node).getId();
				if (lg == 0) {
					//Is a local variable
					var.sym().setoffset(symTab.offset);
					symTab.offset -= 4;
				} else {
					var.sym().setlg(1);
				}
    ...
    public void typeCheck() {
        ...
        for (DeclNode node : myDecls) {
			if (node instanceof FnDeclNode) {
				String name = ((FnDeclNode)node).getId().name();
				if (name.equals("main")) {
					mainExists = true;
				}
			}
...
class FnBodyNode extends ASTnode {
	...
	public int countLocals() {
		return myDeclList.countLocals() + myStmtList.countLocals();
	}
...
class VarDeclNode extends DeclNode {
	...
	//this won't be called by a local variable
	public void codegen(PrintWriter p) {
		myId.isGlobal();
		p.println("\t.data");
		p.println("\t.align 2");
...
class FnDeclNode extends DeclNode {
	...
	public void codegen(PrintWriter p) {
		...
		//Set up stack
		Codegen.genPush(Codegen.RA); //Push ret addr
		Codegen.genPush(Codegen.FP); //Push control link

		int sizeofParams = ((FnSym)myId.sym()).getNumParams() * 4; //gets num params
		Codegen.generate("addu", Codegen.FP, Codegen.SP, 8); 

		int sizeofLocals = myBody.countLocals() * 4; //counts how many locals
		Codegen.generate("subu", Codegen.SP, Codegen.SP, sizeofLocals);
		
		currFn = myId.name(); //remembers current function name
...
abstract class StmtNode extends ASTnode {
	...
	//default countLocals method for nodes without declList | stmtList
	public int countLocals() {
		return 0;
	}
}
...
class PostIncStmtNode extends StmtNode {
	...
	public void codegen(PrintWriter p) {
        ...
        Codegen.genPop(Codegen.T1); //pop value into T1
...
class WriteStmtNode extends StmtNode {
	...
	public void codegen(PrintWriter p) {
		...
		if (expType instanceof StringType) {
			...
			//Int type and bool type will both print out int values
		} else if (expType instanceof IntType || expType instanceof BoolType){
...
class IfStmtNode extends StmtNode {
	...
	public void codegen(PrintWriter p) {
		String notTrueLab = Codegen.nextLabel();
		myExp.codegen(p);
		Codegen.genPop(Codegen.T0);
		Codegen.generate("li", Codegen.T1, 0);
		Codegen.generate("beq",	Codegen.T0, Codegen.T1, notTrueLab);
		myStmtList.codegen(p);
		Codegen.genLabel(notTrueLab);
	}
...
class IfElseStmtNode extends StmtNode {
	...
	public void codegen(PrintWriter p) {
		...
		Codegen.genPop(Codegen.T0);
        Codegen.generate("li", Codegen.T1, 0);
        Codegen.generate("beq", Codegen.T0, Codegen.T1, elseLab);
    ...
    public void nameAnalysis(SymTable symTab) {
        myExp.nameAnalysis(symTab);
        symTab.addScope();
        myThenDeclList.nameAnalysis(symTab, 0);
        ...
        symTab.addScope();
        myElseDeclList.nameAnalysis(symTab, 0);
	...
	public int countLocals() {
		int count = 0;
		count += myThenDeclList.countLocals();
		count += myThenStmtList.countLocals();
		count += myElseDeclList.countLocals();
		count += myElseStmtList.countLocals();
		return count;
	}
...
class ReturnStmtNode extends StmtNode {
	...
	public void codegen(PrintWriter p) {
		...
		Codegen.generate("b", "_" + FnDeclNode.currFn + "_Exit");
	}
...
class StringLitNode extends ExpNode {
	...
	public void codegen(PrintWriter p) {
		...
		p.println(label + ": .asciiz " + myStrVal);
		p.println("\t.text");	
...
class IdNode extends ExpNode {
	...
	//used for fetching the current value of id, and push on stack
	public void codegen(PrintWriter p) {
		if (local) { //it is a local var
			Codegen.generate("lw", Codegen.T0, mySym.getoffset() +"("+Codegen.FP+")");
			Codegen.genPush(Codegen.T0);
		} else { //it is a global var
			Codegen.generate("lw", Codegen.T0, "_" + this.name());		
			Codegen.genPush(Codegen.T0);
		}
	}

	//Create a jum and lin k instruction(op code= jl)
	//main label is just main, otherwise it's _<fnName>
	public void genJumpAndLink() {
		if (this.name().equals("main")) {
			Codegen.generate("jal", this.name());	
		} else {
			Codegen.generate("jal", "_"+this.name());
		}

	}
	
	//Load address into a register and then push it onto the stack
	public void genAddr() {
		if (local) {
			Codegen.generate("la", Codegen.T0, mySym.getoffset() +"("+Codegen.FP+")");
			Codegen.genPush(Codegen.T0);
		} else {
			Codegen.generate("la", Codegen.T0, "_"+this.name());
			Codegen.genPush(Codegen.T0);
		}
	}
...
class NotNode extends UnaryExpNode {
	...
	public void codegen(PrintWriter p) {
		String oneLab = Codegen.nextLabel();
		Codegen.generate("li", Codegen.V1, 0);
		myExp.codegen(p);
		Codegen.genPop(Codegen.T0);
		Codegen.generate("li", Codegen.T1, 1);
		Codegen.generate("beq", Codegen.T0, Codegen.T1, oneLab);
		Codegen.generate("li", Codegen.V1, 1);
		Codegen.genLabel(oneLab);
		Codegen.genPush(Codegen.V1);
    }
...
class TimesNode extends ArithmeticExpNode {
	...
	public void codegen(PrintWriter p) {
        ...
        Codegen.genPop(Codegen.T0);
        Codegen.genPop(Codegen.T1);
        Codegen.generate("multu", Codegen.T0, Codegen.T1);
...
class EqualsNode extends EqualityExpNode {
	...
	public void codegen(PrintWriter p) {
		String equalLab = Codegen.nextLabel();
		Codegen.generate("li", Codegen.V1, Codegen.TRUE);
		myExp1.codegen(p);
        myExp2.codegen(p);
        Codegen.genPop(Codegen.T0);
        Codegen.genPop(Codegen.T1);
        Codegen.generate("beq", Codegen.T0, Codegen.T1, equalLab);
		Codegen.generate("li", Codegen.V1, Codegen.FALSE);
		Codegen.genLabel(equalLab);
		Codegen.genPush(Codegen.V1);
    }
...
class LessNode extends RelationalExpNode {
	...
	public void codegen(PrintWriter p) {
		String lessLab = Codegen.nextLabel();
        Codegen.generate("li", Codegen.V1, Codegen.TRUE);
        myExp1.codegen(p);
        myExp2.codegen(p);
        Codegen.genPop(Codegen.T0);
        Codegen.genPop(Codegen.T1);
        Codegen.generate("blt", Codegen.T1, Codegen.T0, lessLab);
        Codegen.generate("li", Codegen.V1, Codegen.FALSE);
        Codegen.genLabel(lessLab);
        Codegen.genPush(Codegen.V1);
    }
...
class GreaterNode extends RelationalExpNode {
	...
	public void codegen(PrintWriter p) {
		String greatLab = Codegen.nextLabel();
        Codegen.generate("li", Codegen.V1, Codegen.TRUE);
        myExp1.codegen(p);
        myExp2.codegen(p);
        Codegen.genPop(Codegen.T0);
        Codegen.genPop(Codegen.T1);
        Codegen.generate("bgt", Codegen.T1, Codegen.T0, greatLab);
        Codegen.generate("li", Codegen.V1, Codegen.FALSE);
        Codegen.genLabel(greatLab);
        Codegen.genPush(Codegen.V1);
    }
...
class LessEqNode extends RelationalExpNode {
	...
	public void codegen(PrintWriter p) {
		String leqLab = Codegen.nextLabel();
        Codegen.generate("li", Codegen.V1, Codegen.TRUE);
        myExp1.codegen(p);
        myExp2.codegen(p);
        Codegen.genPop(Codegen.T0);
        Codegen.genPop(Codegen.T1);
        Codegen.generate("ble", Codegen.T1, Codegen.T0, leqLab);
        Codegen.generate("li", Codegen.V1, Codegen.FALSE);
        Codegen.genLabel(leqLab);
        Codegen.genPush(Codegen.V1);
    }
...
class GreaterEqNode extends RelationalExpNode {
	...
	public void codegen(PrintWriter p) {
		String geqLab = Codegen.nextLabel();
        Codegen.generate("li", Codegen.V1, Codegen.TRUE);
        myExp1.codegen(p);
        myExp2.codegen(p);
        Codegen.genPop(Codegen.T0);
        Codegen.genPop(Codegen.T1);
        Codegen.generate("bge", Codegen.T1, Codegen.T0, geqLab);
        Codegen.generate("li", Codegen.V1, Codegen.FALSE);
        Codegen.genLabel(geqLab);
        Codegen.genPush(Codegen.V1);
    }
